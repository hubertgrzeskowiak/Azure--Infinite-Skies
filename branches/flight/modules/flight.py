...
DEF_SPECS = {"unloaded_mass":4000.0,
             ...
             #----------------------------------------------
             "Cl_0":0.28, # Lift at zero alpha (CL0)
             "Cd_0":0.03, # Drag at zero alpha (CD0)
             "Cl_a":3.45, # Lift gradient (CLalpha)
             "Cd_a":0.3, # Drag gradient (CDalpha)
             
             "Cl_prate":0.0,
             "Cl_arate":0.72,
             
             "sf_beta":-0.98, # side force from beta
             
             #----------------------------------------------
             # roll 
             #----------------------------------------------
             # basic control effect
             "roll_ail":0.08,
             # basic opposition force
             "roll_damp":-0.26,
             # deviation from flight vector
             "de_beta":-0.12, # dihedral effect
             # other effects
             "roll_rud":-0.105,
             "roll_yawrate":0.14,
             ...
             "pitch_m_arate":-1.1,
             # deviation from flight vector
             "pitch_m_0":0.0,  # reference pitch
             "pitch_m_a":-0.38,  # gradient
             ...
             "yaw_rud":0.032,
             # basic opposition force
             "yaw_damp":-0.35,   # yaw damping
             # opposition from deviation from flight vector
             "wc_beta":0.25,    # weather cocking stability
             # other effects
             "rudder_ay": 0.022, # rudder adverse yaw
             "yaw_ail":0.06,    # yaw generated by ailerons
             
             #----------------------------------------------
             # Control effects on linear forces
             #----------------------------------------------
             "Cl_el":0.36,      # lift from elevator
             "Cd_el":0.1,      # drag from elevator
             "sf_rud":0.1,     # side force from rudder
             ...
             "control_application_rate": [0.06,0.06,0.04],
             "control_return_rate": [0.10,0.10,0.10],
             }
...
class DynamicVariables(object):
    ...
    def exportToPandaNode(self,node):
        """ update a panda node with the current position and angles """
        node.setPos(self.p)
        node.setP(self.ang_p[0])
        node.setR(self.ang_p[1])
        node.setH(self.ang_p[2])
